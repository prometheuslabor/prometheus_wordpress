...
<?php
	...
	echo '<li><span style="color:red;font-weight:bold">ERROR:</span> You are using the SEO plugin <em>' . $seo . '</em>';
	'</em>. You also need to checkmark the <em>Use SEO plugin instead</em> option on the Advanced Options:SEO Weaver II admin tab.</li>';
    }
...
this plugin. Be sure you have filled in the plugin settings.</li>';
    ...
    if (!$seo && weaverii_getopt_checked('_wii_hide_metainfo')) {
	echo '<li>You have checkmarked the <em>Use SEO plugin instead</em> option on the Advanced Options:SEO tab, however none of the more popular SEO plugins were detected. You might want to consider using "WordPress SEO by Yoast", for example.</li>';
    }
...
<meta name=\"keywords\" content=\"$myName blog, $myName\" />";
	...
	$SEOText = str_replace("\n",'',$SEOText);
	if ($meta == $SEOText && !function_exists('wpseo_get_value')) {
