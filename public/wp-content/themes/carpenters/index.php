	...
	<div id="left-div">
		<div id="left-inside">
			...
			<div class="home-post-wrap">
				<div class="heading">
					<span style="font-size: 14px; font-weight: bold;"><?php esc_html_e('Action','Wooden'); ?></span>
				</div>
				...
				if (is_home()) { 
					if (get_option('wooden_duplicate') == 'false') {
						$args=array(
						   'showposts'=> (int) get_option('wooden_homepage_posts'),
						   'post__not_in' => $ids,
						   'paged'=>$paged,
						   'category__not_in' => (array) get_option('wooden_exlcats_recent'),
						);
					} else {
						$args=array(
						   'showposts'=> (int) get_option('wooden_homepage_posts'),
			...
			<div id="sub-post-wrap">
				...
				<ul>
					<?php
						global $post;
						$args = array( 'numberposts' => 5, 'offset'=> 1, 'category' => 5 );
						$myposts = get_posts( $args );
						foreach( $myposts as $post ) :	setup_postdata($post); ?>
							<li><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></li>
						<?php endforeach; ?>
