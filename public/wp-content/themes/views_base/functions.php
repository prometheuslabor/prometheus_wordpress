...
function dequeue_css_if_loaded_from_dB() {
  wp_style_is('views_base');  
  wp_deregister_style('views_base');
}
//Check if CSS is loading from database
$cssfileisloaded=get_option('cssfileisloaded');

if ($cssfileisloaded=='yes') { 
//Stop the loading of parent and child CSS	
add_action('wp_enqueue_scripts','dequeue_css_if_loaded_from_dB');
}
